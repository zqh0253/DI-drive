...
from core.models import CILRSMAEModel
from .base_carla_policy import BaseCarlaPolicy


class CILRSMAEPolicy(BaseCarlaPolicy):
    """
    ...
    def __init__(
        ...
        self._parallel = self._cfg.parallel
        self._max_throttle = self._cfg.max_throttle
        self._model = CILRSMAEModel(**self._cfg.model)
        if self._cuda:
            self._model.cuda()
            if self._parallel:
    ...
    def _forward_eval(self, data: Dict) -> Dict[str, Any]:
        ...
        output = self._model([new_data['rgb']], new_data['speed'], new_data['command'])
        if self._cuda:
    ...
    def _forward_learn(self, data: Dict) -> Dict[str, Any]:
        ...
        output = self._model([rgb], speed, command)
